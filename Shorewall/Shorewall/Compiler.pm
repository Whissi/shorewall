...
sub generate_script_1() {
	...
	if ( $family == F_IPV4 ) {
	    copy $globals{SHAREDIR} . '/prog.header';
	} else {
	    copy $globals{SHAREDIR} . '/prog.header6';
	}
    }
...
EOF
    ...
    unless ( $test ) {
	if ( $family == F_IPV4 ) {
	    copy $globals{SHAREDIR} . '/prog.footer';
	} else {
	    copy $globals{SHAREDIR} . '/prog.footer6';
	}
    }
}
...
sub compiler {
	...
	unless ( $test ) {
	    if ( $family == F_IPV4 ) {
		copy $globals{SHAREDIR} . '/prog.functions';
	    } else {
		copy $globals{SHAREDIR} . '/prog.functions6';
	    }
	}
