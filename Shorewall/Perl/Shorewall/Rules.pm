...
sub process_rule1 ( $$$$$$$$$$$$$$$$ $) {
	...
	my %functions = ( REDIRECT => sub () {
			  ...
			  WHITELIST => sub { fatal_error "'WHITELIST' may only be used in the 'BLACKLIST' section" if $blacklist;
					     $action = 'RETURN'; } ,
			  COUNT => sub { $action = ''; } ,
	...
	unless ( $actiontype & NATONLY ) {
	    ...
	    dont_move $chainref;
	    #
	    # Handle creation of the blacklist chain
	    #
	    if ( $blacklist ) {
	    ...
	    dont_move $chainref if $action eq 'RETURN';
	}
    }
    #
